---
updatedAt: 2026-08-14T06:02:05.000Z
---
    ...
    "rewardDetails": [
      {
        ...
        "termAndConditionsUrl": "https://{host}/{path}",
        "imageId": "EXDR12987U",
        "imageUrl": "https://{host}/{path}",
        "thumbnailId": "EXDR02987T",
        "thumbnailUrl": "https://{host}/{path}",
        "images": [
          {
...
## Reward Redeemed Event (rewardRedeemed)

Notifies when a reward transaction's user rewards move from `ISSUED` to `REDEEMED` through the [Redeem User Reward](https://docs.capillarytech.com/reference/redeem-user-reward) API.

| Parameter           | Data Type | Description                                                                                                                                                   |
| ------------------- | --------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| eventName           | String    | Name of the event. Always `rewardRedeemed`.                                                                                                                   |
| orgId               | Integer   | Organization ID associated with the event.                                                                                                                    |
| refId               | String    | Reference ID for the event, in the format `<orgId>_<transactionId>`.                                                                                          |
| apiRequestId        | String    | Unique identifier for the API request that triggered the redeem.                                                                                              |
| createdAt           | Long      | Timestamp of when the event was created, in Unix epoch milliseconds.                                                                                          |
| data                | Object    | Contains the details of the redeemed reward.                                                                                                                  |
| .userId             | Integer   | Unique identifier of the customer.                                                                                                                            |
| .rewardId           | Integer   | Unique identifier of the reward.                                                                                                                              |
| .rewardExternalId   | String    | External identifier for the reward, if one is mapped. Null if no external ID mapping exists.                                                                  |
| .qty                | Integer   | Number of user rewards transitioned to `REDEEMED` for the transaction.                                                                                        |
| .transactionId      | String    | Unique identifier for the transaction whose rewards were redeemed.                                                                                            |
| .state              | String    | Resulting state of the transaction's user rewards. Always `REDEEMED`.                                                                                         |
| .eventDateTime      | Long      | Date and time at which the redemption happened at the source, in Unix epoch milliseconds. Defaults to the processing time if not provided in the API request. |
| .processingDateTime | Long      | Date and time at which the redeem request was processed, in Unix epoch milliseconds.                                                                          |
| traceAttributes     | Object    | Distributed-tracing attributes carried over from the API request.                                                                                             |
...
```json Webhook payload
{
  "data": {
    ...
    "data": {
      "userId": 656,
      "rewardId": 5105,
      "rewardExternalId": null,
      "qty": 1,
      "transactionId": "9876543210",
      "state": "REDEEMED",
      "eventDateTime": 1778999531000,
      "processingDateTime": 1778999531420
    },
    "orgId": 7351,
    "refId": "7351_9876543210",
    "apiRequestId": "PLINA",
    "createdAt": 1778999531420,
    "traceAttributes": {
      "requestId": "PLINA"
    }
  },
...
## Reward Redemption Reversed Event (rewardRedemptionReversed)

Notifies when a reward transaction's user rewards move from `REDEEMED` back to `ISSUED` through the [Reverse-Redeem User Reward](https://docs.capillarytech.com/reference/reverse-redeem-user-reward) API.

| Parameter           | Data Type | Description                                                                                                                                                 |
| ------------------- | --------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| eventName           | String    | Name of the event. Always `rewardRedemptionReversed`.                                                                                                       |
| orgId               | Integer   | Organization ID associated with the event.                                                                                                                  |
| refId               | String    | Reference ID for the event, in the format `<orgId>_<transactionId>`.                                                                                        |
| apiRequestId        | String    | Unique identifier for the API request that triggered the reversal.                                                                                          |
| createdAt           | Long      | Timestamp of when the event was created, in Unix epoch milliseconds.                                                                                        |
| data                | Object    | Contains the details of the reversed redemption.                                                                                                            |
| .userId             | Integer   | Unique identifier of the customer.                                                                                                                          |
| .rewardId           | Integer   | Unique identifier of the reward.                                                                                                                            |
| .rewardExternalId   | String    | External identifier for the reward, if one is mapped. Null if no external ID mapping exists.                                                                |
| .qty                | Integer   | Number of user rewards transitioned back to `ISSUED` for the transaction.                                                                                   |
| .transactionId      | String    | Unique identifier for the transaction whose redemption was reversed.                                                                                        |
| .state              | String    | Resulting state of the transaction's user rewards. Always `ISSUED`.                                                                                         |
| .actor              | String    | Identifier of the actor who initiated the reversal, echoed from the API request. Null if not provided.                                                      |
| .reason             | String    | Reason for reversing the redemption, echoed from the API request. Null if not provided.                                                                     |
| .eventDateTime      | Long      | Date and time at which the reversal happened at the source, in Unix epoch milliseconds. Defaults to the processing time if not provided in the API request. |
| .processingDateTime | Long      | Date and time at which the reverse-redeem request was processed, in Unix epoch milliseconds.                                                                |
| traceAttributes     | Object    | Distributed-tracing attributes carried over from the API request.                                                                                           |

```json Webhook payload
{
  "data": {
    "eventName": "rewardRedemptionReversed",
    "data": {
      "userId": 656,
      "rewardId": 5105,
      "rewardExternalId": null,
      ...
      "transactionId": "9876543210",
      "state": "ISSUED",
      "actor": "BRAND_OPERATOR",
      "reason": "INCORRECT_REDEMPTION",
      "eventDateTime": 1778999531000,
      "processingDateTime": 1778999531420
    },
    "orgId": 7351,
...
## Reward Revoked Event (rewardRevoked)

Notifies when a reward transaction's user rewards move from `ISSUED` to `CANCELLED` through the [Revoke User Reward](https://docs.capillarytech.com/reference/revoke-user-reward) API.

| Parameter           | Data Type | Description                                                                                                                                               |
| ------------------- | --------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| eventName           | String    | Name of the event. Always `rewardRevoked`.                                                                                                                |
| orgId               | Integer   | Organization ID associated with the event.                                                                                                                |
| refId               | String    | Reference ID for the event, in the format `<orgId>_<transactionId>`.                                                                                      |
| apiRequestId        | String    | Unique identifier for the API request that triggered the revoke.                                                                                          |
| createdAt           | Long      | Timestamp of when the event was created, in Unix epoch milliseconds.                                                                                      |
| data                | Object    | Contains the details of the revoked reward.                                                                                                               |
| .userId             | Integer   | Unique identifier of the customer.                                                                                                                        |
| .rewardId           | Integer   | Unique identifier of the reward.                                                                                                                          |
| .rewardExternalId   | String    | External identifier for the reward, if one is mapped. Null if no external ID mapping exists.                                                              |
| .qty                | Integer   | Number of user rewards transitioned to `CANCELLED` for the transaction.                                                                                   |
| .transactionId      | String    | Unique identifier for the transaction whose rewards were revoked.                                                                                         |
| .state              | String    | Resulting state of the transaction's user rewards. Always `CANCELLED`.                                                                                    |
| .actor              | String    | Identifier of the actor who initiated the revoke, echoed from the API request. Null if not provided.                                                      |
| .reason             | String    | Reason for revoking the reward, echoed from the API request. Null if not provided.                                                                        |
| .eventDateTime      | Long      | Date and time at which the revoke happened at the source, in Unix epoch milliseconds. Defaults to the processing time if not provided in the API request. |
| .processingDateTime | Long      | Date and time at which the revoke request was processed, in Unix epoch milliseconds.                                                                      |
| .pointsReversed     | Number    | Total points refunded across all user rewards in the transaction as part of the revoke. Absent if the revoked rewards had no points attached.             |
| traceAttributes     | Object    | Distributed-tracing attributes carried over from the API request.                                                                                         |

```json Webhook payload
{
  "data": {
    "eventName": "rewardRevoked",
    "data": {
      "userId": 656,
      "rewardId": 5105,
      "rewardExternalId": null,
      ...
      "state": "CANCELLED",
      "actor": "BRAND_OPERATOR",
      "reason": "VENDOR_FULFILMENT_FAILURE",
      "eventDateTime": 1778999531000,
      "processingDateTime": 1778999531420,
      "pointsReversed": 100.0
    },
    "orgId": 7351,
    "refId": "7351_9876543210",
    "apiRequestId": "PLINA",
