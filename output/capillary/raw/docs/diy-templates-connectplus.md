---
updatedAt: 2026-07-03T13:52:32.000Z
---
...
When defining parse path mapping for API responses in your template, you must use the following standardized key names:

* `error_message`
* `entity_id`
* `status_code`

These key names ensure that Connect+ can correctly extract and store values from API responses. If you use different or inconsistent key names, the data will not be stored, even if it is present in the API response. Always map the relevant response fields to these standard keys in your template configuration.
