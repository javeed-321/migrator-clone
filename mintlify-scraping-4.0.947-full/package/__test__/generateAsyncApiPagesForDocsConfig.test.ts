...
describe(generateAsyncApiPagesForDocsConfig, () => {
  ...
  it('writes files directly when no writer is provided', async () => {
    const outDirBasePath = await mkdtemp(join(tmpdir(), 'mint-asyncapi-direct-write-'));

    try {
      await generateAsyncApiPagesForDocsConfig(getAsyncAPIFixturePath('simpleAsyncAPI.yml'), {
        asyncApiFilePath: 'simpleAsyncAPI.yml',
        writeFiles: true,
        outDirBasePath,
        overwrite: true,
      ...
      await expect(
        readFile(join(outDirBasePath, 'websockets/usersignedup.mdx'), 'utf8')
      ).resolves.toBe('---\nasyncapi: /simpleAsyncAPI.yml userSignedUp\n---');
    } finally {
      await rm(outDirBasePath, { recursive: true, force: true });
    }
  });
});
