...
describe("3.6 every spelling goes", () => {
  it("removes the unclosed form, which is a build failure on the target", () => {
    expect(run("One<br>Two").mdx).not.toContain("<br");
  });

  it("removes the self-closed forms too", () => {
    expect(run("One<br/>Two").mdx).not.toContain("<br");
    expect(run("One<br />Two").mdx).not.toContain("<br");
  });

  it("removes the escaped form, which renders as visible literal text", () => {
  ...
  it("removes a <BR> written in capitals", () => {
    expect(run("One<BR>Two").mdx).not.toContain("BR");
  });
});

describe("3.6 nothing takes its place", () => {
  it("keeps one paragraph — no new line, no blank line", () => {
    expect(run("First line.<br />Second line.").mdx).toBe("First line. Second line.");
  });

  it("collapses a run of breaks to a single space", () => {
    expect(run("First line.<br /><br /><br />Second line.").mdx).toBe("First line. Second line.");
  });

  it("does not double the space when one was already there", () => {
    expect(run("First line. <br /> Second line.").mdx).toBe("First line. Second line.");
  });

  it("keeps the words apart rather than colliding them", () => {
    expect(run("alpha<br />beta").mdx).toBe("alpha beta");
  });

  it("removes a trailing break without leaving a space behind", () => {
    expect(run("Only line.<br />").mdx).toBe("Only line.");
  });

  it("removes a leading break", () => {
    expect(run("<br />Only line.").mdx).toBe("Only line.");
  });

  it("removes a break standing alone between blocks, and adds nothing", () => {
    expect(run("## Heading\n\n<br />\n\nA paragraph.").mdx).toBe("## Heading\n\nA paragraph.");
  });

  it("drops a paragraph that held nothing but breaks", () => {
    expect(run("Before.\n\n<br /><br />\n\nAfter.").mdx).toBe("Before.\n\nAfter.");
  });

  it("joins a heading rather than splitting it", () => {
    expect(run("## Rate limits<br />and quotas").mdx).toBe("## Rate limits and quotas");
  });

  it("keeps surrounding inline markup intact", () => {
...
describe("3.6 where it reaches", () => {
  it("strips a break inside a list item, keeping one line", () => {
    expect(run("- Install the CLI.<br />Then run it.").mdx).toBe("- Install the CLI. Then run it.");
  });

  it("strips a break inside a component", () => {
  ...
  it("strips a break nested inside emphasis without breaking the emphasis", () => {
    expect(run("**bold<br />text**").mdx).toBe("**bold text**");
  });

  it("strips a break inside a blockquote", () => {
  ...
  it("leaves a page with no breaks untouched", () => {
  ...
  it("does not touch a <br> written inside a fenced code block", () => {
    const source = "```html\n<p>a<br>b</p>\n```";
...
describe("3.6 through the pipeline", () => {
  it("leaves no <br> anywhere on a page that mixes the spellings", () => {
    const source = [
      "# Rate limits",
      "",
    ...
    const result = convertReadmeMarkdown(source, { title: "Rate limits" });
  ...
  it("leaves the table pass to handle its own cells", () => {
    const source = "| Plan | Limit |\n| --- | --- |\n| Free | 100<br />per minute |";

    expect(convertReadmeMarkdown(source).mdx).toContain("•");
  });

  it("is idempotent", () => {
    const once = convertReadmeMarkdown("One.<br>Two.<br><br>Three.").mdx;

    expect(convertReadmeMarkdown(once).mdx).toBe(once);
  });
});
